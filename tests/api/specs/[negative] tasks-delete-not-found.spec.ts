...
test.describe('[negative] API: Tasks (Swagger) - delete not found', () => {
  ...
  test('returns expected not-found status for DELETE /tasks/{id}', async ({ request, apiData }) => {
    const tasksClient = new TasksClient(request);

    const response = await tasksClient.deleteTask(apiData.notFoundTaskId);
