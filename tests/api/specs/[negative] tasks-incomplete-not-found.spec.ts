...
test.describe('[negative] API: Tasks (Swagger) - incomplete not found', () => {
  ...
  test('returns expected not-found status for POST /tasks/{id}/incomplete', async ({ request, apiData }) => {
    const tasksClient = new TasksClient(request);

    const response = await tasksClient.incompleteTask(apiData.notFoundTaskId);
